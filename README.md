...
- 플레이어 시스템(레벨, 스탯, )
- 아이템 시스템(액티브/패시브)
- 내부 성장 시스템(아이템 획득 및 강화)
- 대화 시스템
- 캐릭터, 아이템, 대화 데이터를 ScriptableObject로 자동 변환
- 프레임 저하 원인 분석 및 최적화 방향 정리
...
아이템 시스템은 정적 데이터와 런타임 상태를 분리하는 방향으로 구현했습니다.

### 주요 구조

| 구성 요소 | 역할 |
|---|---|
| `ItemData` | 아이템의 기본 데이터 관리 |
| `CombinedItemData` | 조합 아이템의 재료 아이템 인덱스 관리 |
| `ItemModuleContainer` | 런타임 아이템 상태 관리 |
| `ItemActiveModule` | 액티브 스킬 관련 동작 관리 |
| `ItemPassiveModule` | 패시브 스탯 효과 관리 |
| `MasterDataManager` | 아이템, 스킬, 성장 데이터 접근 관리 |

### 구현 내용

- ScriptableObject 기반으로 아이템 기본 데이터를 관리했습니다.
- 런타임에서는 `ItemModuleContainer`를 통해 현재 레벨, 최대 레벨, 액티브 / 패시브 모듈을 관리했습니다.
- 아이템의 정적 데이터와 플레이 중 변경되는 런타임 상태를 분리했습니다.
- 패시브 아이템은 플레이어 스탯에 영향을 주도록 구성했습니다.
- 아이템 제거 시 패시브 효과와 지속형 스킬이 남지 않도록 정리 흐름을 검토했습니다.

### 구현 의도

아이템 시스템은 카드 선택, 스킬 발동, 체인 스킬, 조합 아이템과 모두 연결됩니다.  
따라서 단순히 아이템 데이터를 저장하는 구조가 아니라, 런타임에서 안전하게 획득 / 강화 / 제거될 수 있는 구조를 목표로 구현했습니다.
...
조합 아이템은 특정 재료 아이템들이 조건을 만족했을 때 등장하는 특수 아이템입니다.

### 주요 구조

| 구성 요소 | 역할 |
|---|---|
| `CombinedItemData` | 조합 아이템 데이터 |
| `essentialItemIndexes` | 조합에 필요한 재료 아이템 인덱스 목록 |
| `essentialItemNames` | 조합 재료 이름 표시용 데이터 |

### 구현 내용

- 조합 아이템은 일반 아이템 후보와 별도로 관리했습니다.
- 조합 아이템의 재료 아이템이 모두 최대 레벨인지 검사했습니다.
- 조합 조건을 만족한 아이템만 카드 후보에 포함되도록 처리했습니다.
- 이미 보유한 조합 아이템의 재료 아이템은 카드 후보에서 제외되도록 구성했습니다.
- 재료 아이템이 조건을 만족한 첫 순간에는 조합 아이템이 우선 등장할 수 있도록 흐름을 정리했습니다.

### 구현 의도

조합 아이템은 일반 아이템과 같은 방식으로 랜덤 후보에 넣으면 조건 관리가 복잡해집니다.  
그래서 일반 아이템 후보와 조합 아이템 후보를 분리하여 처리했습니다.
...
## 10. Development Build 기반 성능 분석

| 구분 | 내용 |
|---|---|
| 문제 | 에디터가 아닌 실제 빌드 환경에서 프레임 저하 원인을 확인해야 했습니다. |
| 원인 | 에디터 실행 환경과 빌드 실행 환경은 성능 특성이 다르기 때문에, 에디터 프로파일링만으로는 실제 병목을 판단하기 어려웠습니다. |
| 해결 | Development Build와 Profiler를 활용해 실행 중 성능 정보를 확인하는 방향을 정리했습니다. |
| 결과 | 실제 플레이 환경에 가까운 상태에서 렌더링, UI, 카메라, 오브젝트 처리 비용을 분석할 수 있는 흐름을 잡았습니다. |
